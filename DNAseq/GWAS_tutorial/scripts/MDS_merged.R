data<- read.table(file="data/MDS_merge2.mds",header=TRUE)
ancestry<- read.table(file="data/ancestry_file.txt",header=TRUE)
datafile<- merge(data,ancestry,by=c("IID","FID"))
head(datafile)

pdf("results/MDS.pdf",width=7,height=7)
