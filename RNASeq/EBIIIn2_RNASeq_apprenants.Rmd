...
## Hétéroscédasticité des données
...
On utilise les données rendues homoscédastiques grâce à la méthode `rlog()` puis la fonction `plotPCA()` disponible dans DESeq2 :
...
ggplot(d, aes(x=log2FC_4vs0, y=log2FC_8vs0, color=DE_8vs4, size=DE_both_times)) +
  ...
  labs(color="8h vs 4h", size="DE à 4h et à 8h") +
  xlab(bquote(log[2](FC)~"4h vs 0h")) +
  ylab(bquote(log[2](FC)~"8h vs 0h")) +
  ggtitle("Comparaison des 3 conditions") +
  theme_light()
```
...
pheatmap(mat_hm_adj[DE_genes,], 
         ...
         clustering_method="ward.D",
         color=colorRampPalette(rev(brewer.pal(9, "RdBu")))(200),
         main="Heatmap basée sur la matrice variance-stabilisée et ajustée")
```
