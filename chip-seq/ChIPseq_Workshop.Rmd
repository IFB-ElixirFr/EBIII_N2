...
peak centers +/- 150bp:
```{r sox10peaks_300bp, eval=FALSE, cache=TRUE}
...
```{r sox10seq, eval=FALSE, cache=TRUE}
...
```{r export_sox10_seq, eval=2}
## Define the path to the fasta file:
myfastafile = file.path(projPath, "data", "sox10peaks_300bp.fa")
## NOT RUN: export sequences in fasta format
...
`BCrank` takes quite a long time so be patient if you run this command.
```{r bcrank, eval=FALSE, cache=TRUE}
...
```{r bcrank_topmotif, eval=FALSE, cache=TRUE}
topMotif <- toptable(BCRANKout, 1)
```

and extract the Position Weight Matrix (PWM) for this motif and plot the
sequence logo with:
```{r pwm_topmotif_bcrank, eval=FALSE, fig.align='center', cache=TRUE}
pwmnorm_topmotif <- pwm(topMotif, normalize = TRUE)  #with normalize=TRUE the columns will sum to 1 (PFM)
library(seqLogo)
seqLogo(pwmnorm_topmotif)
```
...
```{r motifdb, eval=FALSE, fig.align='center',cache=TRUE}
...
```{r Sox10_PWM, eval=FALSE, cache=TRUE}
...
And then use this PWM to screen the whole genome:
```{r matchPWM_SOX10_Hsapiens, eval=FALSE, cache=TRUE}
SOX10_hg38 <- matchPWM(Sox10JASP22_PWM, Hsapiens, min.score="90%")
```

We can now overlap these motifs with our ChIPseq peaks:
```{r ovl_SOX10hg38_peaks, eval=FALSE, cache=TRUE}
##TODO: countOverlaps?
```
...
with SOX10 ChIP-seq peaks in terms of size and genomic distribution.
